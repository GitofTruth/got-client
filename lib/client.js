...
class Client {
  ...
  // update user >> addUserUpdate
    // create user
  async createNewUser(userName, publicKey = null, privateKey = null){
    try{
  ...
  async updateUserAccess(authorized, access, encryptionKey = null, authorizer = this.user.userInfo.userName){
    try{
        ...
        let response = await this.contract.submitTransaction('updateRepoUserAccess', this.GotReader.getAuthor(), this.GotReader.getName(), authorized, access, authorizer, encryptionKey)
        console.log("Submit Response=",response.toString())
    } catch(e){
        console.log(e)
    }
  ...
  async queryUsersAccess(authorName, repoName){
    try{
