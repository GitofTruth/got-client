...
class Client {
  ...
  constructor(contract) {
    ...
    this.GotReader = new GotReader(branchName())
  ...
  async updateRemote() {
    ...
    let lastCommit =  this.GotReader.getCurrentRemoteBranchHEAD()
    let branchName = this.GotReader.getCurrentBranchName()
    let branchCommits = this.queryBranchCommits(branchName, lastCommit)
  ...
  async remotePostReceive() {
    ...
    for (let i = 0; i < pushlog.logs.length; i++) {
      ...
      await getCommitObjectsHashes(commitHash, allObjectsHashes);
